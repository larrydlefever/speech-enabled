 ...
 *        Assets/SpeechToText/Scripts/Utilities/SavWav.cs
 * 
 *    TODO: consider downloading and building that whole speech-to-text API;
 *          however, seems to require use of standalone IDE (e.g., Xamarin or Visual Studio for Mac),
 *          because need to build it; then, also, need to either manually or by a new script copy the DLL
 *          into this Assembly's Assets folder (or into a sub-folder therein, e.g., "dependencies");
 *          and using Xamarin apparently requires an OSX upgrade; meanwhile, at this point, don't need 
 *          any more than this class
 */
...
namespace UnitySpeechToText.Utilities
{
	...
	public static class SavWav
	{
		...
		{
			var filePath = Path.ChangeExtension(Path.Combine(Path.Combine(Path.Combine(
				Application.dataPath, Constants.SpeechToTextFolderName), Constants.TempFolderName), fileName), "wav");
